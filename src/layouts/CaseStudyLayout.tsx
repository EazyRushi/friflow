...
// Shared reveal hook for case study pages
...
// Renders a stat value with special chars in accent color
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 3, marginTop: 48 }}>
      {items.map((d, i) => (
        <div key={i} className={`del reveal rd${(i % 3) + 1} cursor-none`}
          ...
          onMouseEnter={e => { e.currentTarget.style.background = hoverBg; e.currentTarget.querySelectorAll('.del-icon,.del-t,.del-d').forEach((el: Element) => (el as HTMLElement).style.color = accent === '#FF6B35' ? '#080808' : '#080808') }}
